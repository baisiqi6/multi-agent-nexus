...
- `src/coordinate/split_operations.py` only for neutral pure reconstruction/verification
  helpers.
- `tests/test_projection_doctor.py`.
- focused split-operation tests only if a neutral helper requires direct coverage.

Do not change CLI routing, receipt code, DB schema/migrations, mutation services,
`src/coordinate/cli.py`, or MultiNexus runtime code.
...
- `src/coordinate/cli.py` remains byte-identical to `0563cc0`/`a21d946`.
