...
- `self_test_evidence` schema 显式包含 `raw_e2e_output`（必填原始 e2e 命令输出或日志摘录，不能只是 summary），供 reviewer spot-check。
...
- e2e 失败 → worker 先修 bug，然后重新 self-test 并更新 `self_test_evidence`。
- 无法自行修复或超出 scope → escalate operator（`[agent-report] action=blocker`），不带着失败 e2e 请求 closeout。
- 不允许 "e2e 失败但代码没问题" 式的 closeout；证据必须真实反映当前链路状态。
...
## Reviewer round-1 notes（opencode via reviewer handoff，Approve）

- **软检查依赖 reviewer 自律**（已知，非目标不硬 gate）。
- **非 server task 的 deploy 要求细化**（must-address）：纯文档/测试 task 不需 deploy。bootstrap 自举 checklist 该按 task 类型给——server/daemon/bridge 代码改动 → deploy + e2e；纯文档/测试 → 跳 deploy，自举 = 跑测试。实现时 `_build_worker_bootstrap` 判断 task 改动范围（或让 worker 自填 self_test_evidence 时说明 deploy 与否）。
- **显式 coordinate worktree/branch**（must-address）：worker bootstrap 注明 coordinate repo path + branch（跨 repo task 如 phase-8.5/8.6/8.7），避免 omp 改错分支（phase-8.4.4 的错位重现）。
