# Worker Bootstrap: p9-0a2c-issue-cli
...
You should be at `/opt/multinexus`. If not, `cd /opt/multinexus`.
...
Shared-worktree guard: this checkout may be used by other agents. If `pwd` is not `/opt/multinexus` or `git branch --show-current` is not `feature/multi-bot`, stop and report a blocker instead of switching branches.
...
```bash
cat docs/project-harness/tasks/p9-0a2c-issue-cli/plan.md
```
...
- **Task**: p9-0a2c-issue-cli
- **Title**: P9-0A2c Issue CLI Extraction
- **Branch**: feature/multi-bot
- **Plan**: docs/project-harness/tasks/p9-0a2c-issue-cli/plan.md
- **Phase**: approved
...
```bash
cd /opt/coordinate/.venv/lib/python3.12
PYTHONPATH=src python3 -m coordinate --db /var/lib/coordinate/coord.sqlite3 <command> discord-nexus [options]
```
...
- **Start**: say you accepted `p9-0a2c-issue-cli` and list the 2-3 concrete steps you will do first.
...
```bash
assignment closeout discord-nexus --task-id p9-0a2c-issue-cli --reviewer <reviewer> \
...
- Primary: `/opt/multinexus` branch `feature/multi-bot`
...
3. For implementation tasks, run `assignment closeout discord-nexus --task-id p9-0a2c-issue-cli --reviewer <reviewer>`; do not mark your own implementation done
