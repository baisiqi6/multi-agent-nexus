...
- discord-nexus 只托管 managed coding agents：Claude、Codex、OpenCode。
- 小龙虾/OpenClaw 和 Hermes 是 external gateway agents，不由 discord-nexus 启动。
...
```bash
.venv/bin/python nexus.py --agent mac-claude
.venv/bin/python nexus.py --agent mac-codex
.venv/bin/python nexus.py --agent mac-opencode
```
...
launchd/
  com.discord-nexus.mac-claude.plist
  com.discord-nexus.mac-codex.plist
  com.discord-nexus.mac-opencode.plist
```
...
|---|---|---|
| `mac-claude` | `com.discord-nexus.mac-claude` | `launchd/com.discord-nexus.mac-claude.plist` |
| `mac-codex` | `com.discord-nexus.mac-codex` | `launchd/com.discord-nexus.mac-codex.plist` |
| `mac-opencode` | `com.discord-nexus.mac-opencode` | `launchd/com.discord-nexus.mac-opencode.plist` |
...
<array>
  <string>/Users/yinxin/projects/discord-nexus/.venv/bin/python</string>
  <string>/Users/yinxin/projects/discord-nexus/nexus.py</string>
  <string>--agent</string>
...
<string>/Users/yinxin/projects/discord-nexus</string>
```
...
<string>/Users/yinxin/projects/discord-nexus/logs/mac-claude.log</string>
<key>StandardErrorPath</key>
<string>/Users/yinxin/projects/discord-nexus/logs/mac-claude.err.log</string>
```
...
```bash
com.discord-nexus.${agent}
```

- plist 源路径：

```bash
launchd/com.discord-nexus.${agent}.plist
```

- plist 安装路径：

```bash
~/Library/LaunchAgents/com.discord-nexus.${agent}.plist
```
...
```bash
launchctl bootstrap "gui/$(id -u)" "$HOME/Library/LaunchAgents/com.discord-nexus.mac-claude.plist"
launchctl kickstart -k "gui/$(id -u)/com.discord-nexus.mac-claude"
```
...
```bash
launchctl bootout "gui/$(id -u)/com.discord-nexus.mac-codex"
```
...
```bash
launchctl print "gui/$(id -u)/com.discord-nexus.mac-opencode"
```
...
```text
launchd/com.discord-nexus.mac-claude.plist
launchd/com.discord-nexus.mac-codex.plist
launchd/com.discord-nexus.mac-opencode.plist
```
...
test ! -f "$HOME/Library/LaunchAgents/com.discord-nexus.mac-claude.plist"
```
...
如果用户手动运行 `python nexus.py --agent mac-claude`，同时 launchd 也启动同一个 agent，会出现同 token 双 Gateway 连接风险。
